package me.oggunderscore.Utils;

import org.bukkit.ChatColor;

public class Prefix {
	
	public static String corePrefix = ChatColor.GRAY + "[" + ChatColor.BLUE + "Core" + ChatColor.GRAY + "] ";
	public static String goldPrefix = ChatColor.GRAY + "[" + ChatColor.GOLD + "Gold" + ChatColor.GRAY + "] ";
	public static String errorPrefix = ChatColor.GRAY + "[" + ChatColor.RED + "Error" + ChatColor.GRAY + "] ";
	public static String arenaPrefix = ChatColor.GRAY + "[" + ChatColor.BLUE + "Arena" + ChatColor.GRAY + "] ";
	public static String ffaPrefix = ChatColor.GRAY + "[" + ChatColor.AQUA + "FFA" + ChatColor.GRAY + "] ";
	public static String fightPrefix = ChatColor.GRAY + "[" + ChatColor.AQUA + "Fight" + ChatColor.GRAY + "] ";

}
